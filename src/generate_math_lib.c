...
static Type_Def type_defs[COUNT_TYPES] = {
  [TYPE_FLOAT]        = {.name = "float", .suffix = "", .fmt = "f", .zero_lit = "0.0f"},
  [TYPE_DOUBLE]       = {.name = "double", .suffix = "_f64", .fmt = "lf", .zero_lit = "0.0"},
  [TYPE_INT]          = {.name = "int", .suffix = "_i32", .fmt = "d", .zero_lit = "0"},
  [TYPE_UNSIGNED_INT] = {.name = "unsigned int", .suffix = "_u32", .fmt = "u", .zero_lit = "0u"},
};
...
{
  return shortf("vec%zu%s", n, type_def.suffix);
}
...
#define NUM_VECTOR_COMPS 2
static char *vector_comps[NUM_VECTOR_COMPS][VECTOR_MAX_SIZE] = {
  {"x", "y", "z", "w"},
  {"r", "g", "b", "a"},
};
...
void gen_vector_def(FILE *stream, size_t n, Type_Def type_def)
{
  ...
  fprintf(stream, "  %s all[%lu];\n", type_def.name, n);
  for (size_t c = 0; c < NUM_VECTOR_COMPS; c++) {
    fprintf(stream, "  struct { %s ", type_def.name);
    assert(n <= VECTOR_MAX_SIZE);
    for (size_t i = 0; i < n; ++i) {
      if (i > 0) fprintf(stream, ", ");
      fprintf(stream, "%s", vector_comps[c][i]);
    }
    fprintf(stream, "; };\n");
  }
...
void gen_func_sig_multiple_types (FILE *stream, const char *ret_type, const char *name, const char *arg_types[], const char *arg_names[], size_t arrity) {
  assert(arrity >= 2);
  fprintf(stream, "LADEF %s %s(", ret_type, name);
  for (size_t arg_index = 0; arg_index < arrity; ++arg_index) {
    if (arg_index > 0) fprintf(stream, ", ");
    fprintf(stream, "%s %s", arg_types[arg_index], arg_names[arg_index]);
  }
  fprintf(stream, ")");
}
...
void gen_vector_ctor(FILE *stream, Stmt stmt, size_t n, Type_Def type_def)
{
  ...
  assert(n <= VECTOR_MAX_SIZE);
  gen_func_sig(stream, vector_type.cstr, vector_prefix.cstr, type_def.name, vector_comps[0], n);
...
void gen_vector_scalar_ctor(FILE *stream, Stmt stmt, size_t n, Type_Def type_def)
{
  Short_String vector_type = make_vector_type(n, type_def);
  Short_String vector_prefix = make_vector_prefix(n, type_def);
  Short_String name = shortf("%s_scalar", vector_prefix.cstr);
  static_assert(VECTOR_MAX_SIZE >= 1, "The vector size is too short for this code");
  gen_func_sig(stream, vector_type.cstr, name.cstr, type_def.name, vector_comps[0], 1);
  ...
  if (stmt == STMT_DECL) {
    ...
    for (size_t i = 0; i < n; ++i) {
      if (i > 0) fprintf(stream, ", ");
      static_assert(VECTOR_MAX_SIZE >= 1, "The vector size is too short for this code");
      fprintf(stream, "%s", vector_comps[0][0]);
    }
    fprintf(stream, ");\n");
    fprintf(stream, "}\n");
  } else {
...
{
  Short_String vector_type = make_vector_type(n, type_def);
  Short_String name = shortf("vec%zu%s_%s", n, type_def.suffix, op_def.suffix);
...
void gen_vector_scalar_op(FILE *stream, Stmt stmt, size_t n, Type_Def type_def, Op_Def op_def) {
  Short_String vector_type = make_vector_type(n, type_def);
  Short_String name = shortf("vec%zu%s_%s_scalar",
                             n, type_def.suffix, op_def.suffix);
  const char *arg_types[] = {vector_type.cstr, type_def.name};
  const char *arg_names[] = {"v", "s"};
  gen_func_sig_multiple_types(stream, vector_type.cstr, name.cstr, arg_types, arg_names, 2);
...
Fun_Def fun_defs[COUNT_FUNS] = {
  ...
  [FUN_CLAMP] = {
    ...
    .name_for_type = {
      [TYPE_FLOAT] = "clamp",
      [TYPE_DOUBLE] = "clamp_f64",
      [TYPE_INT] = "clamp_i32",
      [TYPE_UNSIGNED_INT] = "clamp_u32",
    },
    .arity = 3,
    .args = {"x", "a", "b"}
  }
...
void gen_vector_sqrlen(FILE *stream, Stmt stmt, size_t n, Type_Def type_def)
{
  ...
  if (stmt == STMT_DECL) {
    ...
    for (size_t i = 0; i < n; ++i) {
      if (i > 0) fprintf(stream, " + ");
      fprintf(stream, "%s.%s*%s.%s", sqrlen_arg_name, vector_comps[0][i], sqrlen_arg_name, vector_comps[0][i]);
    }
    fprintf(stream, ";\n");
    fprintf(stream, "}\n");
  } else {
...
void gen_vector_printf_macros(FILE *stream, size_t n, Type_Def type_def)
{
  ...
  for (size_t i = 0; i < n; ++i) {
    if (i > 0) fprintf(stream, ", ");
    fprintf(stream, "(v).%s", vector_comps[0][i]);
  }
  fprintf(stream, "\n");
}
                        ...
                        size_t src_n, Type_Def src_type_def)
{
  ...
  Short_String name = shortf("v%zu%s_to_v%zu%s", dst_n, dst_type_def.suffix, src_n, src_type_def.suffix ? src_type_def.suffix : "f32");
...
int main()
{
  ...
  // Header Part
  {
    stream = fopen("./src/la.h", "w");
    if(stream == NULL) {
      fprintf(stderr, "can't create la.h\n");
      exit(1);
    }
    ...
    fprintf(stream, "\n");
    gen_lerp(stream, STMT_DECL, "lerp", "float");
    gen_lerp(stream, STMT_DECL, "lerp_f64", "double");
    ...
    for (Type type = 0; type < COUNT_TYPES; ++type) {
      gen_max(stream, STMT_DECL, type_defs[type]);
      gen_min(stream, STMT_DECL, type_defs[type]);
    ...
    for (size_t n = VECTOR_MIN_SIZE; n <= VECTOR_MAX_SIZE; ++n) {
      for (Type type = 0; type < COUNT_TYPES; ++type) {
        ...
        for (Op_Type op = 0; op < COUNT_OPS; ++op) {
          ...
          gen_vector_scalar_op(stream, STMT_DECL, n, type_defs[type], op_defs[op]);
        }
  ...
  // C part
  {
    stream = fopen("./src/la.c", "w");
    if(stream == NULL) {
      fprintf(stderr, "can't create la.c\n");
      exit(1);
    }
    
    fprintf(stream, "#include \"la.h\"\n");

    gen_lerp(stream, STMT_IMPL, "lerp", "float");
    fputc('\n', stream);
    gen_lerp(stream, STMT_IMPL, "lerp_f64", "double");
    fputc('\n', stream);
    /*
    ...
    */
    //fputc('\n', stream);
    for (Type type = 0; type < COUNT_TYPES; ++type) {
      gen_max(stream, STMT_IMPL, type_defs[type]);
      fputc('\n', stream);
      gen_min(stream, STMT_IMPL, type_defs[type]);
      fputc('\n', stream);
    ...
    for (size_t n = VECTOR_MIN_SIZE; n <= VECTOR_MAX_SIZE; ++n) {
      for (Type type = 0; type < COUNT_TYPES; ++type) {
        ...
        for (Op_Type op = 0; op < COUNT_OPS; ++op) {
          ...
          gen_vector_scalar_op(stream, STMT_IMPL, n, type_defs[type], op_defs[op]);
          fputc('\n', stream);
        }
        for (Fun_Type fun = 0; fun < COUNT_FUNS; ++fun) {
          if (fun_defs[fun].name_for_type[type]) {
